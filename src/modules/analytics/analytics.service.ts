...
export class AnalyticsService {
  ...
  /** The front dashboard: today at a glance. */
  async dashboard(user: AuthenticatedUser, clinicIdParam?: string) {
    ...
    return {
      ...
      visitsByStatus: Object.fromEntries(
        visitsToday.map((v) => [v.status, v._count.status]),
      ),
      appointmentsByStatus: Object.fromEntries(
        appointmentsToday.map((a) => [a.status, a._count.status]),
      ),
      pendingFollowUps,
    };
  }

  /** Daily revenue series between two dates (defaults: last 30 days). */
  async revenue(user: AuthenticatedUser, opts: { from?: string; to?: string; clinicId?: string }) {
    ...
    const series = await this.prisma.$queryRaw<
      { day: Date; revenue: number; payments: number }[]
    >`
  ...
  /** Doctor performance: visits, avg consultation minutes, revenue share. */
  async doctorPerformance(user: AuthenticatedUser, opts: { from?: string; to?: string; clinicId?: string }) {
    const clinicId = resolveClinicId(user, opts.clinicId);
    const to = opts.to ? new Date(opts.to) : new Date();
    const from = opts.from
      ? new Date(opts.from)
  ...
  /** Disease trends: most frequent diagnoses. */
  async diseaseTrends(user: AuthenticatedUser, opts: { from?: string; to?: string; clinicId?: string; limit?: number }) {
    const clinicId = resolveClinicId(user, opts.clinicId);
    const to = opts.to ? new Date(opts.to) : new Date();
    const from = opts.from
      ? new Date(opts.from)
  ...
  /** Most prescribed medicines clinic-wide. */
  async topMedicines(user: AuthenticatedUser, opts: { clinicId?: string; limit?: number }) {
    const clinicId = resolveClinicId(user, opts.clinicId);
  ...
  async followUpRate(user: AuthenticatedUser, clinicIdParam?: string) {
    const clinicId = resolveClinicId(user, clinicIdParam);
